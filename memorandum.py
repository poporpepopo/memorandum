...
# モデルをロードし、それを超えたプロンプトを先頭から約半分まで黙って切り捨てる
# (モデル自体が 128K 対応でも同じ)。先頭には指示文があるため、長い会議の
# 最終要約は指示ごと前半を失う。M1 / 16GB の実測では 60 分の会議 (10,080 トークン)
# が 2,051 トークンまで削られ、終盤の約 12 分しか要約に使われなかった。
# Gemma 4 は num_ctx を 4K から 32K にしても常駐メモリが 3.2GB → 3.4GB と
# ほぼ増えないため、大きく取って会議全体を 1 回で渡す
LLM_NUM_CTX = 32768
# 1 回の生成の上限トークン数。暴走した生成で num_ctx を食い潰さないための上限
LLM_MAX_OUTPUT_TOKENS = 4096
# 1 回の要約に渡す原文の上限 (文字数)。日本語は実測で 1 トークン ≒ 1.9 文字なので
# 約 12,700 トークン (会議で約 75 分)。1 文字 1 トークンの最悪の見積もりでも、
# 指示文と出力の上限を足して LLM_NUM_CTX に収まる。超える会議だけ段階的に要約する
SUMMARY_BLOCK_CHARS = 24000
...
class Summarizer:
    ...
    # 「具体的な内容を省略しない」の一文が無いと、モデルは話題の整理を優先して
    # 日付・金額・数量を落とす。60 分の会議に埋め込んだ 6 つの決定事項で、
    # この一文の有無により要約に残った数が 3/6 → 5/6 に変わった (gemma4:e4b)
    KEEP_SPECIFICS = (
        "決まった日付・金額・数量・担当者などの具体的な内容は、省略せずにすべて書いてください。"
    )
    FINAL_PROMPT = (
        "あなたはプロの書記です。以下の会議の全発言記録から、"
        "重要な論点・決定事項・ネクストアクションを整理した"
        "議事録要約を日本語で作成してください。" + KEEP_SPECIFICS + "\n\n{text}"
    )
    # 原文が 1 回に渡せる量を超えるときの、時間帯ごとの抽出
    SECTION_PROMPT = (
        "以下は会議の一部（{span}）の記録です。この時間帯の重要な論点・"
        "決定事項・ネクストアクションを、箇条書きで抽出してください。"
        + KEEP_SPECIFICS
        + "\n\n{text}"
    )
    ...
    def _chat(self, prompt: str) -> str:
        response = ollama.chat(
            ...
            options={"num_ctx": LLM_NUM_CTX, "num_predict": LLM_MAX_OUTPUT_TOKENS},
        )
